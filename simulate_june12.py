"""
Simulate fund_advisor analysis as if today were 2026-06-08 (Friday).
Key adjustments:
- NAV: filter AKShare time series to <= 2026-06-08
- Stocks (mootdx): skip 4 latest bars (6/12→6/12)
...
def main():
    print("=" * 60)
    print(f"[Simulation] Date: {SIM_DATE} (Friday)")
    print("=" * 60)
    ...
    md_path = os.path.join(OUTPUT_DIR, "daily_report_june12.md")
    with open(md_path, "w", encoding="utf-8") as f:
        f.write(report)
    print(f"[Report] {md_path}")

    json_path = os.path.join(OUTPUT_DIR, "daily_report_june12.json")
